...
namespace Projekt.Intranet.Controllers
{
    public class RezerwacjeController : Controller
    {
        ...
        public RezerwacjeController(FormaContext context)
        {
            _context = context;
        }
        ...
        {
            var rezerwacjeList = await _context.Rezerwacje
                     .Include(t => t.Klient)
                     .Include(t => t.TattoArtists)
                     .ToListAsync();
            var report = new RezerwacjeRaport(rezerwacjeList);
            var pdfBytes = report.GeneratePdf();

            return File(pdfBytes, "application/pdf", "RezerwacjeList.pdf");
        }
    }
}
